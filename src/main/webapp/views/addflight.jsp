...
<body>
	...
	<nav class="navbar navbar-expand-lg navbar-dark">
		<a class="navbar-brand"
			href="${pageContext.request.contextPath}/user/admindashboard">
	...
	<!-- Main Content -->
	<div
		class="container body-content d-flex justify-content-center align-items-center">
		<div class="admin-dashboard-card">
			<h2 class="text-center">Schedule New Flight</h2>
			...
			<form action="${pageContext.request.contextPath}/flight/add"
				method="post">
				<div class="form-group">
					<label for="flightNumber">Flight Number</label> <input type="text"
						class="form-control" id="flightNumber" name="flightNumber"
						required>
				</div>
				<div class="form-group">
					<label for="departureAirport">Departure Airport</label> <select
						class="form-control" id="departureAirport"
						name="departureAirportId" required>
						<c:forEach var="airport" items="${airports}">
							<option value="${airport.airportId}">${airport.airportCode}
								- ${airport.airportName}</option>
						</c:forEach>
					</select>
				</div>
				<div class="form-group">
					<label for="arrivalAirport">Arrival Airport</label> <select
						class="form-control" id="arrivalAirport" name="arrivalAirportId"
						required>
						<c:forEach var="airport" items="${airports}">
							<option value="${airport.airportId}">${airport.airportCode}
								- ${airport.airportName}</option>
				...
				<div class="form-group">
					<label for="departureTime">Departure Time</label> <input
						type="datetime-local" class="form-control" id="departureTime"
						name="departureTime" required>
				</div>
				<div class="form-group">
					<label for="arrivalTime">Arrival Time</label> <input
						type="datetime-local" class="form-control" id="arrivalTime"
	...
	<!-- Custom JavaScript for Sidebar Toggle and Validation -->
	<script>
		...
		function setMinDateTime() {
			...
			document.getElementById('departureTime').setAttribute('min',
					formattedDateTime);
			document.getElementById('arrivalTime').setAttribute('min',
					formattedDateTime);
		}
		...
		function validateDateTimes() {
			...
			if (departureTime && arrivalTime) {
				if (arrivalTime <= departureTime) {
					errorMessage.textContent = 'Arrival time must be after departure time.';
					return false;
				} else {
		...
		function validateAirports() {
			const departureAirport = document
					.getElementById('departureAirport').value;
		...
		window.onload = function() {
			...
			const departureAirportSelect = document
					.getElementById('departureAirport');
			const arrivalAirportSelect = document
					.getElementById('arrivalAirport');

			
			departureTimeInput.addEventListener('change', validateDateTimes);
			arrivalTimeInput.addEventListener('change', validateDateTimes);
			departureAirportSelect.addEventListener('change', validateAirports);
			arrivalAirportSelect.addEventListener('change', validateAirports);

			
			const form = document.querySelector('form');
			form.addEventListener('submit', function(event) {
				if (!validateDateTimes() || !validateAirports()) {
