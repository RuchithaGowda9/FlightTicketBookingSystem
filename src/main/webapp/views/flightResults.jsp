...
<div class="flight-card">
    <div class="flight-card-content">
        <div class="flight-details">
            <h5>Flight Number: ${flight.flightNumber}</h5>
            <p><i class="fas fa-plane-departure"></i> ${flight.departureAirport.airportCode} - ${flight.departureAirport.city} -> ${flight.arrivalAirport.airportCode} - ${flight.arrivalAirport.city}</p>
            <p><i class="fas fa-clock"></i> Departure Time: ${flight.departureTime}</p>
            <p><i class="fas fa-clock"></i> Arrival Time: ${flight.arrivalTime}</p>
            <p><i class="fas fa-money-bill"></i> Price: ${flight.tripPrice}</p>
            <!-- Display message if flight is full -->
            <c:if test="${flight.noOfSeats == 0}">
                <p class="text-danger">Flight Full</p>
            </c:if>
            <c:if test="${flight.noOfSeats >= 1 && flight.noOfSeats <= 15}">
                <p class="text-warning">Hurry Up, Almost Full</p>
            </c:if>
        </div>
    ...
    <form action="${pageContext.request.contextPath}/passengerDetails" method="get">
        <input type="hidden" name="flightId" value="${flight.flightId}">
        <input type="hidden" name="noOfPassengers" value="${param.passengers}">
        <button type="submit" class="btn mt-3 
            ${flight.noOfSeats == 0 ? 'btn-disabled' : 'btn-primary'}" 
            ${flight.noOfSeats == 0 ? 'disabled' : ''}>
            <c:choose>
                <c:when test="${flight.noOfSeats == 0}">
                    Flight Full
                </c:when>
                <c:otherwise>
                    Book Now
