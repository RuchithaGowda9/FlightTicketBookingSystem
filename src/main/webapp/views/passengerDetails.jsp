...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item">
                    <a class="nav-link" href="${pageContext.request.contextPath}/user/userdashboard">
    ...
    <div class="container body-content d-flex justify-content-center align-items-center">
        <div class="passenger-card">
            ...
            <form action="${pageContext.request.contextPath}/submitPassengerDetails" method="post">
                <input type="hidden" name="flightId" value="${param.flightId}">
                <input type="hidden" name="noOfPassengers" value="${noOfPassengers}">
                <c:forEach var="passenger" items="${passengerDetailsList}" varStatus="status">
                    <fieldset class="border p-4 mb-4">
                        <legend class="w-auto">Passenger ${status.index + 1}</legend>
                        <div class="form-group">
                            <label for="firstName${status.index}" class="required-field">First Name</label>
                            <input type="text" id="firstName${status.index}" name="passengerDetailsList[${status.index}].firstName" class="form-control" value="${passenger.firstName}">
                            <div id="firstName${status.index}Error" class="error-message"></div>
                        </div>
                        <div class="form-group">
                            <label for="lastName${status.index}">Last Name</label>
                            <input type="text" id="lastName${status.index}" name="passengerDetailsList[${status.index}].lastName" class="form-control" value="${passenger.lastName}">
                            <div id="lastName${status.index}Error" class="error-message"></div>
                        </div>
                        <div class="form-group">
                            <label for="passportNumber${status.index}" class="required-field">Passport Number</label>
                            <input type="text" id="passportNumber${status.index}" name="passengerDetailsList[${status.index}].passportNumber" class="form-control" value="${passenger.passportNumber}">
                            <div id="passportNumber${status.index}Error" class="error-message"></div>
                        </div>
                        <div class="form-group">
                            <label for="dateOfBirth${status.index}">Date of Birth</label>
                            <input type="date" id="dateOfBirth${status.index}" name="passengerDetailsList[${status.index}].dateOfBirth" class="form-control" value="${passenger.dateOfBirth}">
                        </div>
                        <div class="form-group">
                            <label for="gender${status.index}">Gender</label>
                            <select id="gender${status.index}" name="passengerDetailsList[${status.index}].gender" class="form-control">
    ...
    <script>
        function validateFirstName(input) {
            ...
            if (value.length < 3 || value.length > 50) {
                errorDiv.textContent = 'First name must be between 3 and 50 characters.';
                return false;
            } else {
        ...
        function validateLastName(input) {
            ...
            const errorDiv = document.getElementById(input.id + 'Error');
            if (value.length > 50) {
                errorDiv.textContent = 'Last name must be up to 50 characters.';
                return false;
            } else {
                errorDiv.textContent = '';
                return true;
        ...
        function validatePassportNumber(input) {
            const value = input.value.trim();
            const errorDiv = document.getElementById(input.id + 'Error');
            const regex = /^[A-Z][1-9][0-9]{6}$/;
            if (!regex.test(value)) {
                errorDiv.textContent = 'Passport number must be 8 characters: A-Z, 1-9, followed by any 6 digits.';
                return false;
            } else {
                errorDiv.textContent = '';
                return true;
        ...
        function attachValidation() {
            document.querySelectorAll('input[id^="firstName"]').forEach(input => {
                input.addEventListener('input', () => validateFirstName(input));
            });
            document.querySelectorAll('input[id^="lastName"]').forEach(input => {
                input.addEventListener('input', () => validateLastName(input));
            });
            document.querySelectorAll('input[id^="passportNumber"]').forEach(input => {
                input.addEventListener('input', () => validatePassportNumber(input));
            });
        }
